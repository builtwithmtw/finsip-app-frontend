...
const MonthlyView: React.FC = () => {
    ...
    const { sortedMonths, sortedSymbols, matrix } = useMemo(() => {
        ...
        const matrix: Record<string, Record<string, { shares: number; totalAmount: number; type: string; rawTransactions: Transaction[] }>> = {};

        filteredTransactions.forEach(t => {
            if (!matrix[t.symbol]) matrix[t.symbol] = {};
            if (!matrix[t.symbol][t.month]) {
                matrix[t.symbol][t.month] = { shares: 0, totalAmount: 0, type: t.type, rawTransactions: [] };
            }
                                    ...
                                    {sortedMonths.map(month => {
                                        ...
                                        return (
                                            <td
                                                ...
                                                className="px-5 py-3 border-r border-slate-50 last:border-0 text-center group-hover/row:bg-blue-50/20 transition-colors cursor-pointer hover:!bg-blue-100/50"
                                            >
                                                <div className="flex flex-col items-center">
                                                    <div className={clsx(
                                                        "text-[14px] font-black tracking-tighter leading-none mb-1.5 tabular-nums underline decoration-dotted decoration-slate-300 underline-offset-4",
